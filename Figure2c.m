...
%                        - MATLAB global optimization toolbox
    ...
    %% Load specifications (Ones which do not depend on the Time Horzion): 
        ...
        % Initial condition:   
            ...
            x0 = [0.05;0];
            xtar = [0.2 0.25 -0.3 -0.35 0.4 0.45 ];
...
for i = 1:length(T_array)
    ...
    %% Load specifications (Ones which do depend on the Time Horizon): 
        ...
        % Desired target:
            ...
            xtarget = linspace(xtar(1),xtar(2),T/3)';
            xtarget = [xtarget; linspace(xtar(3),xtar(4),T/3)']; 
            xtarget = [xtarget; linspace(xtar(5),xtar(6),T/3)'];
    ...
    %% Plotting trajectories of each method for a specified Time Horizon: 
        ...
        gbig = kron(g,[1,1])';  
        xtarget = linspace(xtar(1),xtar(2),T_toplot/3)';
        xtarget = [xtarget; linspace(xtar(3),xtar(4),T_toplot/3)']; 
        xtarget = [xtarget; linspace(xtar(5),xtar(6),T_toplot/3)']; 
        plot_markersize = 10;
        plot_fontSize = 10;
        figure(1)
        clf
