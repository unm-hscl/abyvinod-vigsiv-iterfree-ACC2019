function [pwa_time_to_solve,pwa_total_time,pwa_opt_input_vector,...
    ...
    cvx_begin 
        ...
        minimize (trace(cov_X_sans_input(1:state_offset:end,1:state_offset:end))...
            + (xtarget(1:state_offset:end)-pwa_mean_X(1:state_offset:end))'*Q*...
            (xtarget(1:state_offset:end)-pwa_mean_X(1:state_offset:end))...
            +pwa_U_vector'*R*pwa_U_vector)
        subject to
            % (17b)
            sum(pwa_deltai) <= Delta;
            pwa_mean_X == Ad*mean_X_sans_input(1:size(Ad,2))+  Bd * pwa_U_vector;
